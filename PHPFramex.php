...
class DB {
  ...
  public static function _update($params, $item) {
    ...
    // Construir la lista de campos y valores para la actualización
    $updateFields = [];
    ...
    //echo "Params['where']: " . json_encode($params['where']) . "\n";
    
    $query .= " WHERE " . self::conditions($params['where'], ' AND ');
    
    $values = array_values($item);
    $whereValues = array_values($params['where']);
    $values = array_merge($values, $whereValues);
    
    //echo "SQL Query: $query\n";
   // echo "Parameters: " . json_encode($values) . "\n";
...
class Model {
  ...
  public static function update($id,$item) {
    $this->$params = ['table' => static::$table,'where'=>[["AND","id =",$id]]];
